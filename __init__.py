...
class NLPTool:
    ...
    def crfSegment(self, content, inputParams=None):
        ...
        if isinstance(content, str) and len(content)>0:
            ...
            return {'error': { 'content': '長度不得為零的字串'}}
    def nameRecognition(self, content, inputParams=None):
        params = {
            'enablePOSTagging': True,
            'convertMode': '2sc'
        }
        ...
        if isinstance(content, str) and len(content)>0:
            self.generalSetting(params)
            nameSegment = self.HanLP.newSegment().enableNameRecognize(True)
            segments = []
            for v in nameSegment.seg(self._innerConvert(content, '2sc')):
